...
function convertTime(fromTz, toTz, dateStr) {
...
function calculateDate(baseDate, amount, unit, operation, timezone) {
...
function dateDifference(date1Str, date2Str, timezone) {
        ...
        try {
            switch (action) {
                ...
                case 'calculate': {
                    ...
                    const tz = resolveTimezone(parsed.timezone);
                    return calculateDate(parsed.date, parsed.amount, parsed.unit, parsed.operation || 'add', tz);
                }
                case 'difference': {
